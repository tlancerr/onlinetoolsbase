...
export async function POST(req: Request) {
  try {
    ...
    const system =
      "You are a social media growth assistant. Return ONLY valid JSON (no markdown, no extra text).";

    const user = `
Generate Twitter/X hashtags for this keyword: "${keyword}"
...
- No duplicates
- Avoid generic spam tags like #fyp #viral unless truly relevant
- Mix: niche + community + intent + format tags (e.g., #Tips, #Guide, #Thread)
- Keep each hashtag short (ideally <= 18 chars after #)
`;
