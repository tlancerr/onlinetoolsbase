...
async function getCategoryBySlug(slug: string) {
  const data = await fetchWithTimeout(
    `https://cms.cottagecore.pk/wp-json/wp/v2/categories?slug=${encodeURIComponent(slug)}`
  );
...
async function getPostsByCategory(categoryId: number) {
  const data = await fetchWithTimeout(
    `https://cms.cottagecore.pk/wp-json/wp/v2/posts?categories=${categoryId}&per_page=20&_embed=1`
  );
