"use client";

import { useState } from "react";
import ToolLayout from "../../../components/ToolLayout";

export default function CalorieCalculator() {
  const [age, setAge] = useState("");
  const [height, setHeight] = useState("");
  const [weight, setWeight] = useState("");
  const [gender, setGender] = useState("male");
  const [activity, setActivity] = useState("1.2");
  const [result, setResult] = useState<string | null>(null);

  function calculateCalories() {
    if (!age || !height || !weight) {
      setResult("Please fill all fields.");
      return;
    }

    const a = parseInt(age);
    const h = parseFloat(height);
    const w = parseFloat(weight);

    let BMR = 0;

    if (gender === "male") {
      BMR = 88.36 + (13.4 * w) + (4.8 * h) - (5.7 * a);
    } else {
      BMR = 447.6 + (9.2 * w) + (3.1 * h) - (4.3 * a);
    }

    const calories = Math.round(BMR * parseFloat(activity));

    setResult(
      `Your estimated daily calorie needs are ${calories} calories per day.`
    );
  }

  return (
    <ToolLayout
      title="Calorie Calculator"
      description="Calculate your daily calorie needs based on age, height, weight, gender, and activity level."
      category="Health and Fitness Tools"
    >
      <div className="space-y-4">

        <div>
          <label className="tool-label">Age (years)</label>
          <input
            type="number"
            className="tool-input"
            value={age}
            onChange={(e) => setAge(e.target.value)}
          />
        </div>

        <div>
          <label className="tool-label">Height (cm)</label>
          <input
            type="number"
            className="tool-input"
            value={height}
            onChange={(e) => setHeight(e.target.value)}
          />
        </div>

        <div>
          <label className="tool-label">Weight (kg)</label>
          <input
            type="number"
            className="tool-input"
            value={weight}
            onChange={(e) => setWeight(e.target.value)}
          />
        </div>

        <div>
          <label className="tool-label">Gender</label>
          <select
            className="tool-input"
            value={gender}
            onChange={(e) => setGender(e.target.value)}
          >
            <option value="male">Male</option>
            <option value="female">Female</option>
          </select>
        </div>

        <div>
          <label className="tool-label">Activity Level</label>
          <select
            className="tool-input"
            value={activity}
            onChange={(e) => setActivity(e.target.value)}
          >
            <option value="1.2">Sedentary (little to no exercise)</option>
            <option value="1.375">Lightly Active (1–3 days/week)</option>
            <option value="1.55">Moderately Active (3–5 days/week)</option>
            <option value="1.725">Very Active (6–7 days/week)</option>
            <option value="1.9">Super Active (heavy physical job)</option>
          </select>
        </div>

        <button onClick={calculateCalories} className="btn-primary">
          Calculate Calories
        </button>

        {result && (
          <div className="mt-4 rounded-lg bg-slate-900 border border-slate-700 px-3 py-3 text-sm text-emerald-300">
            {result}
          </div>
        )}
      </div>
    </ToolLayout>
  );
}
