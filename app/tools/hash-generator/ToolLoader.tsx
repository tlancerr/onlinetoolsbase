...
async function hashString(text: string, algo: "MD5" | "SHA-256") {
  if (algo === "MD5") {
    // Simple JS MD5 implementation (not cryptographically perfect, but ok for a tool)
...
export default function ToolLoader() {
  ...
  async function handleGenerate() {
    if (!text) {
      setHash("Enter some text to hash.");
      return;
    }
    const h = await hashString(text, algo);
    setHash(h);
  }
  ...
  return (
      ...
      <div className="space-y-4">
        ...
        <select
          ...
          value={algo}
          onChange={(e) => setAlgo(e.target.value as "MD5" | "SHA-256")}
        >
        ...
        {hash && (
          <textarea
            className="tool-input h-24 mt-3 text-emerald-300"
            readOnly
            value={hash}
          />
        )}
